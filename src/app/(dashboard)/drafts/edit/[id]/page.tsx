...
export default function EditDraftPage({ params: { id } }: Props) {
  ...
  const [updateDraft, { isLoading: isUpdatingDraft }] =
    useUpdateDraftMutation();
  ...
  const defaultValues: TPostForm = useMemo(() => {
    if (isDraftSuccess) {
      return {
        asEvergreen: draftData?.data?.asEvergreen ?? false,
        isDraft: true,
        onLinkedIn: draftData?.data?.onLinkedIn ?? false,
        onTwitter: draftData?.data?.onTwitter ?? false,
        scheduleDate: new Date().toISOString(),
        isTemplate: false,
        addFinisher: false,
        posts:
          draftData?.data?.posts.map((post, index) => ({
            ...post,
            images: [],
            index,
          })) ?? [],
  ...
  }, [currentAccount, draftData, isDraftSuccess]);
  ...
  const handleUpdateDraft = useCallback(async () => {
    ...
    form.getValues("posts").forEach((post, index) => {
      data.append(`Posts[${index}].id`, id);
    });

    const saveDraftPromise = updateDraft({
      id,
      body: data,
    }).unwrap();
  ...
  return (
    <div className="space-y-2 px-4 py-4 md:px-8">
      ...
      {isDraftLoading ? (
                  ...
                  <div>
                    <div>
                      ...
                      >
                        <Button
                          type="button"
                          disabled={!form.formState.isValid || isUpdatingDraft}
        ...
        <ErrorCard
          title="Failed to fetch draft"
          refetchFunction={refetchDraft}
        />
      )}
    </div>
  );
