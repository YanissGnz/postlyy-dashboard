...
export default function EditTemplatePage({ params: { id } }: Props) {
  ...
  const defaultValues: TPostForm = useMemo(() => {
    if (isTemplateSuccess) {
      return {
        asEvergreen: templateData?.data?.asEvergreen ?? false,
        isDraft: false,
        onLinkedIn: templateData?.data?.onLinkedIn ?? false,
        onTwitter: templateData?.data?.onTwitter ?? false,
        scheduleDate: new Date().toISOString(),
        isTemplate: false,
        addFinisher: false,
        posts:
          templateData?.data?.posts.map((post, index) => ({
            ...post,
            images: [],
            index,
          })) ?? [],
  ...
  }, [currentAccount, templateData, isTemplateSuccess]);
  ...
  }, [templateData, isTemplateSuccess]);
  ...
  const handleUpdateTemplate = useCallback(async () => {
    ...
    const saveTemplatePromise = updateTemplate({
      id,
      body: data,
    }).unwrap();
  ...
  return (
    <div className="space-y-2 px-4 py-4 md:px-8">
      <div className="mb-5 flex items-center justify-between">
        <h2 className="text-2xl font-bold">Create Templates</h2>
      </div>
      {isTemplateLoading ? (
        <LoadingCard />
      ) : isTemplateSuccess ? (
                    ...
                    <div>
                      ...
                      >
                        <Button
                          type="button"
                          disabled={
                            !form.formState.isValid || isUpdatingTemplate
                          }
        ...
        <ErrorCard
          ...
          refetchFunction={refetchTemplate}
        />
      )}
    </div>
  );
